...
\input partmac
\endtt
For \LaTeX users, I've prepared the file "strdiag.sty", which includes Ti$k$Z automatically, so it is sufficient to write
\begtt
\usepackage{strdiag}
\endtt
...
\bigskip\hfil\table{lclclclclc}{
"\Dcup"   & \Dcup    & "\Dcap"  & \Dcap & "\Did" & \Did & "\Dcross" & \Dcross\cr
"\Rcap"   & \Rcap    & "\Lcap"  & \Lcap & "\Fid" & \Fid & "\Bid"    & \Bid    & "\Fcross" & \Fcross\cr
}
